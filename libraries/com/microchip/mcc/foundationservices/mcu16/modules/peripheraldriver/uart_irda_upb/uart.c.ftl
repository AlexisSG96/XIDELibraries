...
void __attribute__ ((vector(_UART${instanceNumber}_TX_VECTOR), interrupt(IPL${transmitInterrupt.priorityValue}SOFT))) ${transmitInterrupt.handlerName} ( void )
{
...
{ 
    ...
    ${interruptTransmit.clearFlag};

    while(!(${StatusReg}bits.UTXBF == 1))
    {
        ${UTXREG} = *txHead++;

        if(txHead == (txQueue + ${data.moduleName}_CONFIG_TX_BYTEQ_LENGTH))
        {
            txHead = txQueue;
        }
...
void __attribute__ ((vector(_UART${instanceNumber}_RX_VECTOR), interrupt(IPL${receiveInterrupt.priorityValue}SOFT))) ${receiveInterrupt.handlerName}( void )
{
...
void ${moduleNameUpperCase}_Receive_ISR(void)
{
    while((${StatusReg}bits.URXDA == 1))
    {
        *rxTail = ${URXREG};
        ...
        if ( ( rxTail    != (rxQueue + ${data.moduleName}_CONFIG_RX_BYTEQ_LENGTH-1)) &&
             ((rxTail+1) != rxHead) )
        {
            rxTail++;
        } 
        else if ( (rxTail == (rxQueue + ${data.moduleName}_CONFIG_RX_BYTEQ_LENGTH-1)) &&
                  (rxHead !=  rxQueue) )
        {
      ...
      ${interruptReceive.clearFlag};
}
</#if>

<#if errorInterrupt??>
void __attribute__ ((vector(_UART${instanceNumber}_ERR_VECTOR), interrupt(IPL${errorInterrupt.priorityValue}SOFT))) ${errorInterrupt.handlerName} ( void )
{
    <#assign reg = data.getRegisterByAlias("USTA")>
...
void _mon_putc(char c) {
    while(${data.moduleName}_IsTxReady() == false)
    {
    }
  
    ${data.moduleName}_Write(c);
}
</#if>
