...
typedef struct { uint16_t con1; uint16_t brg; uint8_t operation;} ${moduleNameLowerCase}_configuration_t;
...
<#list spiConfgurationValues?keys as config>
    { ${spiConfgurationValues[config]["CON1L"]}, ${spiConfgurationValues[config]["BRGL"]}, ${spiConfgurationValues[config]["OPERATION"]} }<#if config?has_next>,</#if>
</#list>
};
