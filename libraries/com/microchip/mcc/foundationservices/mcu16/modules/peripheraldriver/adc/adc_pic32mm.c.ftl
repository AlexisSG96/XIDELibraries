...
void ${data.moduleName}_Start(void)
{
   ...
   ${reg.name}SET = (1 << _${reg.name}_${reg.getSettingByAlias("SAMP").name}_POSITION);
}
void ${data.moduleName}_Stop(void)
{
   <#assign reg = data.getRegisterByAlias("ADCON1")>
   ${reg.name}CLR = (1 << _${reg.name}_${reg.getSettingByAlias("SAMP").name}_POSITION);
}
...
<#if settings.useInterrupt>
void __attribute__ ( ( vector ( _ADC_VECTOR ), interrupt ( IPL${interruptadc.priorityValue}SOFT ) ) )ADC${instanceNumber}_ISR (void)
<#else>
void ${data.moduleName}_Tasks ( void )
</#if>
{
    // clear the ADC interrupt flag
    ${interruptadc.clearFlag};
}
