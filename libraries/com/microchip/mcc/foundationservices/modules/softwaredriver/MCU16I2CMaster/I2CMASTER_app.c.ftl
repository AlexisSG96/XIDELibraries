...
#include "i2c_types.h"
#include "../${i2cFunctions["i2cHeader"]}"
...
void ${i2cMasterFunctions["poller"]}(void);
...
i2c_error_t ${i2cMasterFunctions["open"]}(i2c_address_t address)
{
    ...
    if(!${i2cMasterFunctions["status"]}.inUse)
    {
        ...
        ${i2cFunctions["setMasterI2cISR"]}(${i2cMasterFunctions["masterISR"]});
        ${i2cFunctions["setSlaveI2cISR"]}(${i2cMasterFunctions["masterISR"]});
...
static i2c_fsm_states_t do_I2C_SEND_ADR_READ(void)
{
    ...
    ${i2cFunctions["TXData"]}(${i2cMasterFunctions["status"]}.address << 1 | 1);
    return I2C_RCEN;
}
...
{
    ${i2cMasterFunctions["status"]}.addressNACKCheck = 1;
    ${i2cFunctions["TXData"]}(${i2cMasterFunctions["status"]}.address << 1);
    return I2C_TX;
}
...
static i2c_fsm_states_t do_I2C_TX(void)
{
    ...
    {
        ${i2cMasterFunctions["status"]}.addressNACKCheck = 0;
        ${i2cFunctions["TXData"]}(*${i2cMasterFunctions["status"]}.data_ptr++);
