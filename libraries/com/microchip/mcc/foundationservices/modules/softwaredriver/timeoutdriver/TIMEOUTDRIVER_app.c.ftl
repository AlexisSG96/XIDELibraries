...
inline void setTimerDuration(uint32_t duration)
{
    lastTimerLoad = ${TimerMaxCount} - duration;

    ${WriteTimer}(0);

    ${ClearIntFlag};
    ${WriteTimer}(lastTimerLoad);
}
...
{    
    ...
    {
        timerAbsoluteTime += ${TimerMaxCount} - rebaseList() + 1;
    ...
    {
        setTimerDuration(${TimerMaxCount});
        ${ClearIntFlag};
...
void startTimerAtHead(void)
{
    ...
    if (period > ${TimerMaxCount})
    {
        dummy.absoluteTime = absoluteTimeofLastTimeout + ${TimerMaxCount};
        dummy.next = listHead;
        listHead = &dummy;
        period = ${TimerMaxCount};
    }
...
inline void enqueueCallback(timerStruct_t* timer)
{
    ...
    if( timer == &dummy )
    {
        timer = &dummy_exec;// keeping a separate copy for dummy in execution queue to avoid the modification of next from the timer list. <#--For more info: MCU8CC-14201  -->
    }
    timer->next = NULL;
...
{
    ${SetInterruptHandler}(timeout_isr);
}
