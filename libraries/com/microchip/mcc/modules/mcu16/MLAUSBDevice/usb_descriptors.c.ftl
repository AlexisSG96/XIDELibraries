...
/** INCLUDES *******************************************************/
#include "usb.h"
#include "usb_device_<#if DeviceClass == "CDC">cdc<#elseif DeviceClass == "Vendor">generic</#if>.h"
...
const USB_DEVICE_DESCRIPTOR device_dsc=
{
    ...
    0x${BCDUSB?remove_beginning("0x")?left_pad(4, "0")?upper_case},                 // USB Spec Release Number in BCD format
    <#if DeviceClass == "CDC">CDC_DEVICE,             // Class Code
<#elseif DeviceClass == "Vendor">0x00,                   // Class Code
</#if>
    0x00,                   // Subclass code
    0x00,                   // Protocol code
    USB_EP0_BUFF_SIZE,      // Max packet size for EP0, see usb_device_config.h
    0x${DeviceVID?remove_beginning("0x")?left_pad(4, "0")?upper_case},                 // Vendor ID
    0x${DevicePID?remove_beginning("0x")?left_pad(4, "0")?upper_case},                 // Product ID
    0x${BCDDevice?remove_beginning("0x")?left_pad(4, "0")?upper_case},                 // Device release number in BCD format
    0x0<#if ManufacturerString?length != 0>1<#else>0</#if>,                   // Manufacturer string index
    0x0<#if ProductString?length != 0 && ManufacturerString?length != 0>2<#elseif ProductString?length != 0 && ManufacturerString?length == 0>1<#else>0</#if>,                   // Product string index
    0x0<#if SerialNumberString?length != 0 && ManufacturerString?length != 0 && ProductString?length != 0>3<#elseif SerialNumberString?length != 0 && (ManufacturerString?length != 0 || ProductString?length != 0)>2<#elseif ManufacturerString?length == 0 && ProductString?length == 0 && SerialNumberString?length != 0>1<#else>0</#if>,                   // Device serial number string index
...
const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[${ManufacturerString?length}];}sd001={
sizeof(sd001),USB_DESCRIPTOR_STRING,
{${ManufacturerString?replace("","','")?keep_after("',")?keep_before_last(",'")}}
};

<#else></#if>
<#if ProductString?length != 0>
//Product string descriptor
const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[${ProductString?length}];}sd00<#if ManufacturerString?length != 0>2<#else>1</#if>={
sizeof(sd00<#if ManufacturerString?length != 0>2<#else>1</#if>),USB_DESCRIPTOR_STRING,
{${ProductString?replace("","','")?keep_after("',")?keep_before_last(",'")}}
};
...
const struct{uint8_t bLength;uint8_t bDscType;uint16_t string[${SerialNumberString?length}];}sd00<#if ManufacturerString?length != 0 && ProductString?length != 0>3<#elseif ManufacturerString?length != 0 || ProductString?length != 0>2<#else>1</#if>={
sizeof(sd00<#if ManufacturerString?length != 0 && ProductString?length != 0>3<#elseif ManufacturerString?length != 0 || ProductString?length != 0>2<#else>1</#if>),USB_DESCRIPTOR_STRING,
{${SerialNumberString?replace("","','")?keep_after("',")?keep_before_last(",'")}}
};
...
{
    (const uint8_t *const)&sd000<#if ManufacturerString?length != 0 || ProductString?length != 0 || SerialNumberString?length != 0 >,
    (const uint8_t *const)&sd001<#else></#if><#if (ManufacturerString?length != 0 && ProductString?length != 0) || (ManufacturerString?length != 0 && SerialNumberString?length != 0) || (ProductString?length != 0 && SerialNumberString?length != 0)>,
    (const uint8_t *const)&sd002<#else></#if><#if ManufacturerString?length != 0 && ProductString?length != 0 && SerialNumberString?length != 0>,
    (const uint8_t *const)&sd003<#else></#if>
