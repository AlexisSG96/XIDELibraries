...
<#if DeviceClass == "CDC">            CDCInitEP();
<#elseif DeviceClass == "Vendor"></#if>
            break;
        ...
        case EVENT_EP0_REQUEST:
            <#if DeviceClass == "CDC">/* We have received a non-standard USB request.  The CDC driver
             * needs to check to see if the request was for it. */
            USBCheckCDCRequest();<#elseif DeviceClass == "Vendor">/* We have received a non-standard USB request.  The vendor driver
             * needs to check to see if the request was for it. */
            USBCheckVendorRequest();</#if>
            break;
...
<#if usb.DeviceName?contains("PIC24") || usb.DeviceName?contains("dsPIC33")>
...
<#if usb.DeviceName?contains("PIC32MM")>
<#if InterruptMode == "Interrupt">
void __attribute__((vector (_USB_VECTOR), interrupt(IPL${usb.priority}SOFT))) USB_ISR()
{
    USBDeviceTasks();
}
</#if>
