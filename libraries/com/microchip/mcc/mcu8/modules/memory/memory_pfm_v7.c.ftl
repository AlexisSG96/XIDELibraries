...
int8_t FLASH_WriteBlock(uint32_t writeAddr, uint8_t *flashWrBufPtr)
{
    ...
    for(j=0; j<numberOfWriteBlocks; j++)
    {
        ...
        ${TablePointerUpperByte} = (uint8_t)(((writeAddr + WriteBlkOffset) & 0x00FF0000) >> 16);    // Load Table point register
        ${TablePointerHighByte} = (uint8_t)(((writeAddr + WriteBlkOffset) & 0x0000FF00) >> 8);
        ${TablePointerLowByte} = (uint8_t)((writeAddr + WriteBlkOffset) & 0x000000FF);
        
        // Write block of data
        for (i=0; i<WRITE_FLASH_BLOCKSIZE; i++)
        {
            ${TableLatchRegister} = flashWrBufPtr[WriteBlkOffset+i];  // Load data byte
