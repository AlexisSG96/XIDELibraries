/*
<#include "MicrochipDisclaimer.ftl">
*/

#ifndef DRIVER_MRF24J40MA_H
#define	DRIVER_MRF24J40MA_H

#include <stdint.h>
#include <stdbool.h>
<#if (isAVR == "true")>
#include "config/clock_config.h"
#include "util/delay.h"
</#if>
        
/*
 *    |-------------------|------------------------------------------------------|---------------|
 *    |    1    |    1    |    2    |    1   |   2    |   2    |   2    |   2    |    N Bytes    |
 *    |---------|---------|---------|--------|--------|--------|--------|--------|---------------|
 *    | Header  | Frame   |  Frame  |   SEQ  |  DEST  |  DEST  |  SRC   |  SRC   |     Data      |
 *    | Length  | Length  | Control |   NUM  |  PAN   |  ADR   |  PAN   |  ADR   |               |
 *    |---------|---------|---------|--------|--------|--------|--------|--------|---------------|
 *                        |<----------------------- Header --------------------->|<-- Payload -->|
 */
#define HEADER_LENGTH                11
#define TX_FIFO_FIXED_DATA_LENGTH    13

/*
 *   |---------|-----------------|-----------|--------|--------|--------|
 *   |    1    |        11       |    n      |   2    |   1    |    1   |
 *   |---------|-----------------|-----------|--------|--------|--------|
 *   |  Frame  |     Header      |  Payload  |  FCS   |  LQI   |  RSSI  |
 *   | Length  |                 |           |        |        |        |
 *   |---------|-----------------|-----------|--------|--------|--------|
 */
#define RX_FIFO_FIXED_DATA_LENGTH    16

#define RX_FIFO_ADDR                 0x300        //With reference to FIGURE 2-6 in MRF24J40MA datasheet
#define TX_NORMAL_FIFO_ADDR          0x000        //With reference to FIGURE 2-6 in MRF24J40MA datasheet
#define RX_FIFO_SIZE                 0x90        //With reference to FIGURE 2-6 in MRF24J40MA datasheet
#define TX_NORMAL_FIFO_SIZE          0x80        //With reference to FIGURE 2-6 in MRF24J40MA datasheet

//
// SHORT ADDRESS CONTROL REGISTER MAP FOR MRF24J40MA with reference to w.r.t. Figure 2.11 in MRF24J40MA data sheet
//
#define RXMCR                        0x00
#define PANIDL                       0x01
#define PANIDH                       0x02
#define SADRL                        0x03
#define SADRH                        0x04
#define EADR0                        0x05
#define EADR1                        0x06
#define EADR2                        0x07
#define EADR3                        0x08
#define EADR4                        0x09
#define EADR5                        0x0A
#define EADR6                        0x0B
#define EADR7                        0x0C
#define RXFLUSH                      0x0D
#define ORDER                        0x10
#define TXMCR                        0x11
#define ACKTMOUT                     0x12
#define ESLOTG1                      0x13
#define SYMTICKL                     0x14
#define SYMTICKH                     0x15
#define PACON0                       0x16
#define PACON1                       0x17
#define PACON2                       0x18
#define TXBCON0                      0x1A
#define TXNCON                       0x1B
#define TXG1CON                      0x1C
#define TXG2CON                      0x1D
#define ESLOTG23                     0x1E
#define ESLOTG45                     0x1F
#define ESLOTG67                     0x20
#define TXPEND                       0x21
#define WAKECON                      0x22
#define FRMOFFSET                    0x23
#define TXSTAT                       0x24
#define TXBCON1                      0x25
#define GATECLK                      0x26
#define TXTIME                       0x27
#define HSYMTMRL                     0x28
#define HSYMTMRH                     0x29
#define SOFTRST                      0x2A
#define SECCON0                      0x2C
#define SECCON1                      0x2D
#define TXSTBL                       0x2E
#define RXSR                         0x30
#define INTSTAT                      0x31
#define INTCON_M                     0x32
#define GPIO                         0x33
#define TRISGPIO                     0x34
#define SLPACK                       0x35
#define RFCTL                        0x36
#define SECCR2                       0x37
#define BBREG0                       0x38
#define BBREG1                       0x39
#define BBREG2                       0x3A
#define BBREG3                       0x3B
#define BBREG4                       0x3C
#define BBREG6                       0x3E
#define CCAEDTH                      0x3F

//
// LONG ADDRESS CONTROL REGISTER MAP FOR MRF24J40MA with reference to w.r.t. Figure 2.12 in MRF24J40MA data sheet
//
#define RFCON0                       0x200
#define RFCON1                       0x201
#define RFCON2                       0x202
#define RFCON3                       0x203
#define RFCON5                       0x205
#define RFCON6                       0x206
#define RFCON7                       0x207
#define RFCON8                       0x208
#define SLPCAL0                      0x209
#define SLPCAL1                      0x20A
#define SLPCAL2                      0x20B
#define RFSTATE                      0x20F
#define RSSI                         0x210
#define SLPCON0                      0x211
#define SLPCON1                      0x220
#define WAKETIMEL                    0x222
#define WAKETIMEH                    0x223
#define REMCNTL                      0x224
#define REMCNTH                      0x225
#define MAINCNT0                     0x226
#define MAINCNT1                     0x227
#define MAINCNT2                     0x228
#define MAINCNT3                     0x229
#define ASSOEADR0                    0x230
#define ASSOEADR1                    0x231
#define ASSOEADR2                    0x232
#define ASSOEADR3                    0x233
#define ASSOEADR4                    0x234
#define ASSOEADR5                    0x235
#define ASSOEADR6                    0x236
#define ASSOEADR7                    0x237
#define ASSOSADR0                    0x238
#define ASSOSADR1                    0x239
#define UPNONCE0                     0x240
#define UPNONCE1                     0x241
#define UPNONCE2                     0x242
#define UPNONCE3                     0x243
#define UPNONCE4                     0x244
#define UPNONCE5                     0x245
#define UPNONCE6                     0x246
#define UPNONCE7                     0x247
#define UPNONCE8                     0x248
#define UPNONCE9                     0x249
#define UPNONCE10                    0x24A
#define UPNONCE11                    0x24B
#define UPNONCE12                    0x24C

#define SRC_SHORT_ADDRESS            ${sourceAddress}
#define SRC_PAN_ID                   0x0303
#define DEST_SHORT_ADDRESS           ${destinationAddress}
#define DEST_PAN_ID                  0x0303

typedef enum
{
    //This provides value of RFCON0 register
    //Value of RFCON3:RFCON0 is considered as 0x3 (Recommended)
    CHANNEL_2_405GHZ =  0x03,
    CHANNEL_2_410GHZ =  0x13,
    CHANNEL_2_415GHZ =  0x23,
    CHANNEL_2_420GHZ =  0x33,
    CHANNEL_2_425GHZ =  0x43,
    CHANNEL_2_430GHZ =  0x53,
    CHANNEL_2_435GHZ =  0x63,
    CHANNEL_2_440GHZ =  0x73,
    CHANNEL_2_445GHZ =  0x83,
    CHANNEL_2_450GHZ =  0x93,
    CHANNEL_2_455GHZ =  0xA3,
    CHANNEL_2_460GHZ =  0xB3,
    CHANNEL_2_465GHZ =  0xC3,
    CHANNEL_2_470GHZ =  0xD3,
    CHANNEL_2_475GHZ =  0xE3,
    CHANNEL_2_480GHZ =  0xF3
} rf_channel_t;

typedef enum
{
    //This provides value of BBREG2 register
    //CCACSTH<3:0> is considered 0b1110 as recommended
    CCA_MODE_1 = 0x80,
    CCA_MODE_2 = 0x40,
    CCA_MODE_3 = 0xC0
}cca_mode_t;

typedef enum
{
    //This provides value of BBREG6 register
    RSSI_MODE_1 = 0x80,
    RSSI_MODE_2 = 0x40,
}rssi_mode_t;

typedef enum
{
    RX_ALL = 0x0E,
    RX_CMD_ONLY = 0x06,
    RX_DATA_ONLY = 0x0A,
    RX_BCN_ONLY = 0x0C
}rxfrm_filter_t;

typedef enum
{
    RX_MODE_NORMAL = 0x03,
    RX_MODE_ERROR = 0x10,
    RX_MODE_PROMISCUOUS = 0x01
}rx_mode_t;

typedef enum
{
    NONBEACON_PAN_CORRD,
    NONBEACON_COORD,
    NONBEACON_DEV
}node_type_t;

void MRF24J40MA_SelectChannel(rf_channel_t channel);
void MRF24J40MA_SelectCCAMode(cca_mode_t mode);
void MRF24J40MA_SelectRSSIMode(rssi_mode_t mode);
bool MRF24J40MA_ReadRxFIFO(uint8_t *rxData, uint8_t length);
void MRF24J40MA_WriteTxNormalFIFO(uint8_t *txPayload, uint8_t payloadLen);
void MRF24J40MA_SetPANID(uint16_t panid);
void MRF24J40MA_SetShortAddress(uint16_t address);
void MRF24J40MA_SetTXPower(uint8_t power);
void MRF24J40MA_ControlPLL(bool state);
void MRF24J40MA_SetRxFrameFilter(rxfrm_filter_t filter);
void MRF24J40MA_SetReceptionMode(rx_mode_t mode);
void MRF24J40MA_SetNodeType(node_type_t type);
void MRF24J40MA_ControlAckRequest(bool state);
void MRF24J40MA_ControlSecurity(bool state);
uint8_t MRF24J40MA_ReadShortRegister(uint8_t address);
void MRF24J40MA_WriteShortRegister(uint8_t address, uint8_t data);
uint8_t MRF24J40MA_ReadLongRegister(uint16_t address);
void MRF24J40MA_WriteLongRegister(uint16_t address, uint8_t data);
void MRF24J40MA_RFReset(void);
void MRF24J40MA_SoftwareReset(void);
void MRF24J40MA_HardwareReset(void);
void MRF24J40MA_Init(void);

#endif	/* DRIVER_MRF24J40MA_H */
