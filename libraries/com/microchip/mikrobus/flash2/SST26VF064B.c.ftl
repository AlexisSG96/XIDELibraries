...
{
    ${SPIFunctions["exchangeByte"]}(command);
...
{
    value = ${SPIFunctions["exchangeByte"]}( 0 );
...
void SST26VF064B_SPI_ReadBuffer( uint8_t *buffer, uint16_t count )
{
    while( count-- )
    {   
        *( buffer++ ) = ${SPIFunctions["exchangeByte"]}( 0x00 );
    }
...
void SST26VF064B_SPI_WriteBuffer( uint8_t *buffer, uint16_t count )
{
    while( count-- )
    {
        ${SPIFunctions["exchangeByte"]}( *( buffer++ ) );          
    }
...
{    
    ...
    ${SPIFunctions["exchangeByte"]}( addr_hi );
    ${SPIFunctions["exchangeByte"]}( addr_mi );
    ${SPIFunctions["exchangeByte"]}( addr_lo );
