...
static uint8_t rtcc_read(uint8_t addr) {
    return ${I2CFunctions["read1ByteRegister"]}(RTCC_ADDR, addr);
}

static void rtcc_write(uint8_t addr, uint8_t data) {
    ${I2CFunctions["write1ByteRegister"]}(RTCC_ADDR, addr, data);
    <#if (isAVR == "true")>
    _delay_ms(5);
	<#else>
    __delay_ms(5);
...
static void rtc6_EEPRWriteLatchEnable(void) {
    ${I2CFunctions["write1ByteRegister"]}(EEADDR, STATUS, EEPROM_UNLOCK);
}

static void rtc6_EEPRWriteLatchDisable(void) {
    ${I2CFunctions["write1ByteRegister"]}(EEADDR, STATUS, EEPROM_LOCK);
}

static uint8_t rtc6_EEPRReadStatusRegister(void) {
    ${I2CFunctions["read1ByteRegister"]}(EEADDR, STATUS);
}
...
uint8_t rtc6_ReadByteEEPROM(uint8_t addr) {
    ...
    return ${I2CFunctions["read1ByteRegister"]}(EEADDR, addr);
}
...
void rtc6_WriteByteEEPROM(uint8_t addr, uint8_t data) {
    ...
    ${I2CFunctions["write1ByteRegister"]}(EEADDR, addr, data);
