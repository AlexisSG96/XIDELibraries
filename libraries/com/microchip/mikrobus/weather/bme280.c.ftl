...
void BME280_writeByte(uint8_t reg_addr, uint8_t data)
{
    ${I2CFunctions["write1ByteRegister"]}(BME280_ADDR, reg_addr, data);
}

void BME280_writeBlock(uint8_t *write_buff, uint8_t length)
{
    ${I2CFunctions["writeNBytes"]}(BME280_ADDR, write_buff, length);
}

uint8_t BME280_readByte(uint8_t reg_addr)
{
    return ${I2CFunctions["read1ByteRegister"]}(BME280_ADDR, reg_addr);
}

void BME280_readBlock(uint8_t reg_addr, uint8_t *read_buff, uint8_t length)
{
    ${I2CFunctions["readDataBlock"]}(BME280_ADDR, reg_addr, read_buff, length);
}
